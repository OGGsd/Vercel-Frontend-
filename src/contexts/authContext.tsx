...
export function AuthProvider({ children }): React.ReactElement {
  ...
  function login(
    ...
    // Set cookies with 24-hour expiration for better persistence
    ...
    cookies.set(LANGFLOW_AUTO_LOGIN_OPTION, autoLogin, {
      path: "/",
      expires,
      secure: window.location.protocol === 'https:',
      sameSite: 'lax'
    ...
    setLocalStorage("auth_timestamp", Date.now().toString());

    if (refreshToken) {
      cookies.set(LANGFLOW_REFRESH_TOKEN, refreshToken, {
        path: "/",
        expires,
        secure: window.location.protocol === 'https:',
        sameSite: 'lax'
      });
      setLocalStorage(LANGFLOW_REFRESH_TOKEN, refreshToken);
    }
