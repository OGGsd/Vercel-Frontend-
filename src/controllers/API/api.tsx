...
function ApiInterceptor() {
  ...
  useEffect(() => {
    ...
    const interceptor = api.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        const isTokenExpired = authTimestamp ?
          (Date.now() - parseInt(authTimestamp)) > (24 * 60 * 60 * 1000) : true;

        const shouldRetryRefresh =
          isAuthenticationError &&
          !isTokenExpired &&
          !isLoginPage &&
          ((IS_AUTO_LOGIN === false) || (!autoLogin && autoLogin !== undefined));
    ...
    const requestInterceptor = api.interceptors.request.use(
      async (config) => {
        ...
        try {
          ...
          // Suppress request errors in console for security
          if (process.env.NODE_ENV === 'development') {
            console.debug('Request error (hidden in production):', error.message);
          }
        }
  ...
  function checkErrorCount() {
    ...
    // Allow only 1 retry attempt to prevent spam
    if (authenticationErrorCount > 1) {
  ...
  async function tryToRenewAccessToken(error: AxiosError) {
    ...
    mutationRenewAccessToken(undefined, {
      ...
      onError: (error) => {
        // Suppress authentication errors in console for security
        if (process.env.NODE_ENV === 'development') {
          console.debug('Authentication error (hidden in production)');
        }
        mutationLogout();
        // Suppress promise rejection to prevent console error
